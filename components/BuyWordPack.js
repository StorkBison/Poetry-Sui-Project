    ...
    <section className="bg-gray-600/70 w-full fixed top-0 left-0">
      <div className="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0 w-full">
        <div className="p-2 md:mt-0 xl:p-4 transparent text-white w-full h-full m-6">
          {
            isMinting ?
              <Image priority alt="Picture"
                src={background === "starter" ? Starter : background === "jumbo" ? Jumbo : Colossal}
                className='w-full' />
              :
              (
                <div className='m-8'>
                  <div className="flex items-start justify-between p-4">
                    <h1 className="mb-4 text-4xl md:text-6xl tracking-tight uppercase text-center w-full">New Words Available</h1>
                    <button onClick={() => setShowPoem( false )} type="button" className="bg-transparent hover:text-gray-300 p-1.5 ml-auto inline-flex items-center" data-modal-hide="defaultModal">
                      <svg className='w-16 h-16' fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                      ...
                      words.map( ( item, index ) => (
                        ...
                        <div key={index}
                          style={{
                            ...
                            transform: `matrix(1, ${ ( Math.random() * ( 8 + 8 ) - 8 ) / 100 }, 0, 1, 0, 0)`,
