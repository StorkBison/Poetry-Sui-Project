  ...
  return (
    <div>
      <button className="cursor-pointer p-2 border-meet draw meet" onClick={handleShowPoem}>
        <Image priority alt="Picture" src={poem.imageUrl} width={360} height={360} className='w-full border border-gray-400' />
        <p className="font-light sm:text-xl">TITLE: {poem.title}</p>
        <p className="font-light sm:text-xl">CREATED: {month[ ( new Date( poem.createdAt ).getMonth() ) ]} {new Date( poem.createdAt ).getDate()}, {new Date( poem.createdAt ).getFullYear()}</p>
      </button>
      {
        showPoem &&
        <PoemModal
